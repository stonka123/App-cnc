...
const kindProcess: HTMLOptionElement = document.querySelector('#kind')
...
const constValueVc: number = 318

const calculatetest = () => {
	let convertNumToStr: string = diameter.value
	let processingType = Number(kindProcess.value)
	let diameterTools = Number(diameter.value)
	let materialType = Number(material.value)

	let score: number = ((processingType * constValueVc) / diameterTools) * materialType
...
const checkInfo = (processingType: number, diameterTools: number, materialType: number) => {
	if (!diameterTools) {
...
const checkString = (convertNumToStr: string) => {
